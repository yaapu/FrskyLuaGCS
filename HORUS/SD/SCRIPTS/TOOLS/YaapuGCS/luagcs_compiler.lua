...
local function createEmptyFiles(pageType)
  ...
  pf = assert(io.open(libBasePath.."qplane_"..pageType.."_1.lua","a+"))
  if pf ~= nil then
    io.close(pf)
  end

  pf = assert(io.open(libBasePath.."plane_"..pageType.."_1.lua","a+"))
  if pf ~= nil then
    io.close(pf)
  end

  pf = assert(io.open(libBasePath.."copter_"..pageType.."_1.lua","a+"))
  if pf ~= nil then
    io.close(pf)
  end

  pf = assert(io.open(libBasePath.."heli_"..pageType.."_1.lua","a+"))
  if pf ~= nil then
    io.close(pf)
  end

  pf = assert(io.open(libBasePath.."rover_"..pageType.."_1.lua","a+"))
  if pf ~= nil then
    io.close(pf)
  end
end
...
local function run(event)
  if LCD_W > 212 then
    lcd.setColor(CUSTOM_COLOR, lcd.RGB(8,84,136))
