...
local function drawWarning(text)
  ...
  lcd.drawText(60, 29, text, INVERS)
end
local function drawBars(page, menu)
  local itemIdx = string.format("%s %d/%d",string.sub(page.description,1,14),menu.selectedItem,#page.list)
...
local function drawTopBar(status,telemetryEnabled,telemetry)
  ...
  lcd.drawText(LCD_W-40,0, string.format("Bat:%.01fV",telemetry.batt1volt*100), SMLSIZE+INVERS+RIGHT)  
end
...
local function drawCommandItem(items,idx,menu,msgRequestStatus,mavResult)
  lcd.drawText(0,7 + (idx-menu.offset-1)*7, items[idx][1],SMLSIZE)
  ...
  lcd.drawText(LCD_W-42,7 + (idx-menu.offset-1)*7, items[idx][2][items[idx].value],flags+SMLSIZE+RIGHT)
...
local function drawListItem(items,idx,menu,msgRequestStatus)
  ...
  if items[idx].value == nil then
    lcd.drawText(LCD_W-42,7 + (idx-menu.offset-1)*7, "---",SMLSIZE+flags+RIGHT)
  else
    if type(items[idx][2]) == "table" then -- COMBO
      lcd.drawText(LCD_W-42,7 + (idx-menu.offset-1)*7, items[idx][2][items[idx].value],SMLSIZE+flags+RIGHT)
    else
      lcd.drawText(LCD_W-42,7 + (idx-menu.offset-1)*7, string.format(items[idx].fstring,items[idx].value,(items[idx][5]~=nil and items[idx][5] or "")),flags+SMLSIZE+RIGHT)
    end
  end
  lcd.drawText(LCD_W,7 + (idx-menu.offset-1)*7, msgRequestStatus[items[idx].status],flags+SMLSIZE+RIGHT)
end
